...
- **素材**：列出照片、音乐、歌词并指出缺什么。缺音频可以直接搜索并下载（需要 yt-dlp），
  缺歌词可以在线找一份 `.lrc`，也可以让 tsuzuri 本地识别。
- **制作**：按素材与依赖的实际状态决定能不能开工——不能时会写明缺什么、怎么补，而不是
  只把按钮灰掉。可以选参数（EXIF / 签名 / 黑底 / 画幅 / 滤镜 / 草稿 / 时长裁剪）后直接开工，
  过程中能看进度、随时取消；旁边同时给出等效的终端命令，可以复制走。滤镜有实时预览，
  用的是与成片同一份滤镜定义（浏览器与渲染器对 SVG 滤镜有细微差异，以成片为准）。
- **成果**：看成片、跟播歌词（可点行跳转）、翻照片（大图带 EXIF 展签）。

同一时刻只跑一个任务，第二个请求会被拒绝。所有写操作都要带 server 启动时生成的 token，
且路径限制在启动时锁定的沙箱根内。除任务本身产生的成片/照片外，只有 `/api/thumb` 的
缩略图缓存会写入系统临时目录，不往素材夹里放额外文件。
...
`--filter <id>` 和 `--filter-intensity <0-1>`（默认按滤镜自带强度）为 still 与视频渲染通用的滤镜开关，内置：`faded`（褪色）、`warm`（暖阳）、`cool`（冷调）、`mono`（黑白）、`vintage`（怀旧）、`vignette`（暗角）、`teal-orange`（青橙）、`riso`（孔版）、`film`（胶片褪色）。裸命令菜单在渲染/still 流程里也会问一步「选择滤镜」，回车即为无滤镜。

逐张滤镜可写进素材夹根目录的 `tsuzuri.json`（不存在则跳过，不影响原有流程）：
