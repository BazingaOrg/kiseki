...
把三批过程中"当时明确接受"的已知限制逐条做掉:

- **停滞看门狗（fetch-audio）**：卡在 0% 的下载(代理挂了但 TCP 不断)会永远占着
  并发锁,用户不点取消就再也起不了任何任务。2 分钟没有任何进度事件即判定卡死、
  说明原因并中止。**只给 fetch-audio 挂** —— whisper 识别本来就会安静好几分钟再
  一次性吐结果,给它挂必然误杀;有一条专门的测试守住这个区别。
- **killAll 时的临时目录残留**：Ctrl+C 之后进程立刻就走,`'exit'` 回调没机会跑,
  `finalize` 里的 `rmSync` 也就不执行,下载中途关掉 server 会把 `/tmp/tsuzuri-fetch-*`
  留在磁盘上。spec 现在把 `tempDir` 暴露出来,`killAll` 同步删掉。
- **任务表无限增长**：只保留最近 20 条已结束的记录,正在跑的绝不动。
- **缩略图缓存无上限**：缓存键含 mtime,原图改一次就多留一份旧的;浏览多个大素材夹
  也会堆积。超过 2000 个文件时按最后访问时间砍到 80%,且只在**真正新生成了一张**
  之后才检查 —— 命中缓存的请求(绝大多数)一次 `readdir` 都不用付。

停滞阈值与轮询间隔做成可注入参数,否则单测得干等两分钟。

验证:`cd cli && node --test` **355/355**;真进程实测 killAll —— 下载中途调用,
中转目录与 yt-dlp 进程双双归零(假 child 抓不到这两类,必须用真进程)。

### 仍然没做的

- **音视频的实际播放**：自动化浏览器媒体解码不可用,需要在常规 Chrome 里确认。
  这是唯一一条从批 A 起就一直欠着、且我无法自行验证的。

### 过程教训（第三次踩同一个坑）

这一轮又出现一次"字符串替换锚点没匹配、静默什么都没做",导致常量定义没插进去、
21 个测试一起红。前两次分别发生在 `Workbench.tsx` 的锁定说明和同一批常量上。
**结论:不要对改过多次的文件做盲替换;先 grep 确认锚点存在,改完再验证结果。**

## 真实使用暴露的问题（2026-07-25，用户实测）

用户在常规 Chrome 里跑了一遍,报了三条。**两条是真 bug,一条是我要澄清的误解** ——
而这三条没有一条是自动化验证能抓到的。

### 【高】播放时崩溃:在 setState 的 updater 里读 currentTarget

渲染完成后切到成果段,`onLoadedMetadata` 一触发就抛
`Cannot read properties of null (reading 'duration')`,整棵组件树被打断。

`event.currentTarget` 写在 `setState` 的 updater 函数里,而那个函数是 React 在
**渲染阶段**才调用的 —— 那时事件处理早已返回,React 把 `currentTarget` 重置成了
null。`onTimeUpdate` 同理。改成先解构出值再进 setState。

批 A 就写下的错误。自动化浏览器里媒体解码整体不可用,`loadedmetadata` 从来没
真正触发过 —— 这正是那条"音视频实际播放需要在常规 Chrome 里确认"的遗留项
一直悬着的代价。

### 【中】歌词显示不全:两个原因叠在一起

1. **素材卡只列前 3 行**。刚下完歌词的人看到的就是"不全"。素材段的用途正是确认
   自己拿到的是哪一份歌词,那就该列全;超高自己滚。
2. **更隐蔽的**:那份 lrc 有 22 行时间戳,其中 5 行只有时间没有文本,
   `parseLrc` 的 `if (!content) continue` 直接丢掉 → 只剩 17 行。空行在 LRC 里是
   "上一句到此为止"的标记(间奏、留白),丢掉的后果是 01:07 那句一直高亮到 01:21,
   整个间奏都不消失。

第 2 点**没有改 parseLrc 的默认行为** —— 对视频字幕来说丢掉空行是对的(空字幕
不该显示)。加 `keepGaps` 选项只给 web API 用,`withGapEnds` 把空行折叠成上一句的
`until`,自己不出现在结果里(否则列表会多出一堆 ⋯)。CLI 与渲染器一个字节没变。

### 澄清:渲染时风扇狂转不是网页端的性能问题

核过证据:`buildJobSpec` 拼的 argv 直接喂 `cli/tsuzuri.mjs`,**与终端里跑是同一条
代码路径、同一套参数**。默认并发是 `parallelism - 1`(10 核机器上用 9 核),
非草稿模式满分辨率 + crf 16。风扇是渲染本身的固有成本,终端里跑一模一样。

已有旋钮 `TSUZURI_CONCURRENCY`(正整数或百分比),而且**能透传到网页任务** ——
`spec.env` 继承 `process.env`,实测 `TSUZURI_CONCURRENCY=3` 确实传到了子进程。
所以 `TSUZURI_CONCURRENCY=4 tsuzuri web` 现在就能用。

缺的是网页里没有这个控件,只能在启动时设。是否加进制作段表单待定。

### 教训

三条里有两条真 bug,都出在**只有真人在真浏览器里用才会走到的路径**上:
媒体解码、以及"下载一份真实的 lrc 而不是测试夹具"。自动化验证、单测、我自己的
浏览器实测都没碰到。这和之前 `cli/cli/` 那次是同一个形状,只是又往外扩了一圈:
**mock 挡住的地方、环境不支持的地方、以及用测试夹具替代真实数据的地方,
是同一类盲区。**
